...
- Logo components — `Logo` (icon mark) and `LogoFull` (icon + wordmark), SVG with `fill="currentColor"` for automatic theme adaptation; `LogoMark` (`@stasho/ds/logo-mark`) is the badge variant — real outlines, fixed brand palettes, deliberately theme-independent because it is exported to PNG/ICO (Decision #106)
