...
- Radius vocabulary 0/0/2/4 — `full` reserved for StatusDot, Slider thumb (principled aperture per Decision #89), RadioGroup item (round-by-design), ProgressBar tracks; Switch track + thumb moved to `rounded-[2px]` (Decision #88); Stepper indicators likewise; Tabs pill moved to `rounded-[2px]` (Decision #86); MultiSelect chips moved to `rounded-[2px]` (Decision #91)
